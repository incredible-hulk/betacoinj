...
import com.google.betacoin.store.H2FullPrunedBlockStore;
import com.google.betacoin.utils.BlockFileLoader;
...
/**
 * A tool for comparing the blocks which are accepted/rejected by bitcoind/betacoinj
 * It is designed to run as a testnet-in-a-box network between a single bitcoind node and betacoinj
...
public class BitcoindComparisonTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof BlockAndValidity) {
                ...
                if (!chain.getChainHead().getHeader().getHash().equals(bitcoindChainHead)) {
                    differingBlocks++;
                    log.error("bitcoind and betacoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
        ...
        log.info("Done testing.\n" +
                "Blocks which were not handled the same between bitcoind/betacoinj: " + differingBlocks + "\n" +
