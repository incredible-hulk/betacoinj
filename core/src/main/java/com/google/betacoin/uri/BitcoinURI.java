 ...
 *
 * <ul>
 * <li>{@code betacoin:<address>}</li>
 * <li>{@code betacoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
...
public class BitcoinURI {
    ...
    public static final String BITCOIN_SCHEME = "betacoin";
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        // URI is formed as  betacoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form bitcoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the betacoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : betacoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("betacoin://")) {
            schemeSpecificPart = input.substring("betacoin://".length());
        } else if (input.startsWith("betacoin:")) {
            schemeSpecificPart = input.substring("betacoin:".length());
        } else {
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the betacoin: prefix");
